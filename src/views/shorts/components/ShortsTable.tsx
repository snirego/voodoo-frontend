...
const NameColumn = ({ row }: { row: Customer }) => {
    ...
    return (
        <div className="flex items-center" onClick={() => console.log("Open Dialog")}>
                {row.name}
        </div>
    )
}

const SchduledToColumn = ({ row }: { row: Customer }) => {

    return (
        <div className="flex items-center">
            <Badge className={statusColor[row.status]} />
            <span className="ml-2 rtl:mr-2 capitalize">
            {row.status === 'scheduled' ? dayjs.unix(row.lastOnline).format('MM / DD / YYYY') :
            'Paused'}
            </span>
        </div>
    )
    ...
    const columns: ColumnDef<Customer>[] = useMemo(
        () => [
            ...
            },
            {
                ...
                accessorKey: 'status',
                cell: (props) => 
                <SchduledToColumn row={props.row.original} />
            },
            {
                header: '',
                id: 'action',
